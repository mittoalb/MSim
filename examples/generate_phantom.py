...
def create_metadata_json(lookup_dict, voxel_size, output_path):
    ...
    Parameters:
    - lookup_dict: dict mapping label IDs to material properties
    - voxel_size: (z, y, x) voxel dimensions in micrometers
...
def create_sphere_phantom(shape=(128, 128, 128), voxel_size=(0.5, 0.5, 0.5)):
    """Create a simple sphere phantom."""
    nz, ny, nx = shape
    volume = np.zeros(shape, dtype=np.int32)
...
if __name__ == "__main__":
    ...
    if phantom_type not in ["sphere", "cylinder", "bone", "microstructure"]:
        print("Usage: python generate_phantom.py [sphere|cylinder|bone|microstructure]")
        print("Available phantoms:")
        print("  sphere         - Simple nested spheres")
        print("  cylinder       - Cylindrical features")
        print("  bone           - Complex bone with implants, contrast, calcifications")
    ...
    print(f"\nTo test simulation:")
    print(f"from interface import quick_tomography")
    print(f"projections = quick_tomography('{zarr_path}', '{json_path}', n_projections=90)")
    ...
    print(f"\nMaterial properties:")
    for label, props in meta["lookup"].items():
        if props.get("composition"):
            formula = ''.join(f"{el}{amt}" for el, amt in props["composition"].items())
            print(f"  Label {label}: {formula}, ρ={props['density']} g/cm³")
        else:
            print(f"  Label {label}: vacuum/air")