...
// now with MAX_BRANCH_LEN to cap each branch
double draw_vessels(
    uint8_t*    mask,
    uint8_t*    occ,  // occupancy mask
    ...
    // 1) carve the straight trunk (with periodic jitter)
    ...
    for(int t = 0; t < TRUNK_LEN; ++t) {
        // jitter trunk direction every JITTER_INTERVAL steps
        ...
        if (!occ[off]) {
            carve_ball(mask, occ, nz, ny, nx, zi, yi, xi, base_radius, 7, total_length);
        }
        pz += fdz;
        py += fdy;
        px += fdx;
    ...
    while (!stack.empty()) {
        ...
        if (n.depth >= max_depth || n.idx + 3 > rng_len)
            continue;
        ...
        // compute forward-limit and cap branch length
        ...
        for (int i = 0; i < length; ++i) {
            ...
            if (!occ[off2]) {
                carve_ball(mask, occ, nz, ny, nx, zi2, yi2, xi2, int(n.radius), 7, total_length);
            }
        }
...
//──────────────────────────────────────────────────────────────────────────────
// add_vessels: plant many roots, carve them into labels[], return total length
// now samples per-vessel radius around average
